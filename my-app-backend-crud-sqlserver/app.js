...
import dotenv from 'dotenv';// to read environment variables from a .env file
...
app.use('/api/v1', orderRoutes); // Order routes

// Default root route
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'public/index.html')); // Serve the main HTML file information above the directory declared
});
...
app.listen(port, () => {
  ...
  console.log(`Advanced operations with JOIN queries -> http://localhost:${port}/orders.html`);
});
